  ...
  if (form) {
    form.addEventListener('submit', function(e) {
      ...
      if (!uid || !uid.value || !uid.value.trim()) {
        ...
        if (wayfarer.showAlert) wayfarer.showAlert('danger', 'Please select a user to invite.');
      } else {
        ...
        fetch(url, {
          ...
          .then(function(data){
            if (data && data.success) {
              ...
              if (wayfarer.showAlert) wayfarer.showAlert('success', 'Invitation sent.');
              // clear search fields
              if (searchInput) searchInput.value = '';
              results.innerHTML = '';
              uid.value = '';
            } else {
              if (wayfarer.showAlert) wayfarer.showAlert('danger', (data && data.message) || 'Failed to send invite.');
            }
          })
          .catch(function(err){ if (wayfarer.showAlert) wayfarer.showAlert('danger', 'Failed to send invite: ' + err); });
      }
    });
  }
                  ...
                  .then(function(data){
                    if (data && data.success) {
                      if (f.action.endsWith('RemoveMember')) {
                        ...
                        if (tr) tr.remove();
                        if (wayfarer.showAlert) wayfarer.showAlert('success', 'Invitation canceled.');
                      ...
                      if (wayfarer.showAlert) wayfarer.showAlert('danger', (data && data.message) || 'Action failed.');
                    }
                  })
                  .catch(function(err){ if (wayfarer.showAlert) wayfarer.showAlert('danger', 'Action failed: ' + err); });
              } else {
                f.submit();
              }
            }
