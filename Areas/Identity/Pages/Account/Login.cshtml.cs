...
namespace Wayfarer.Areas.Identity.Pages.Account
{
        ...
        public async Task<IActionResult> OnPostAsync(string? returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    //// Check if the NameIdentifier claim exists, if not, add it
                    //if (!claims.Any(c => c.Type == ClaimTypes.NameIdentifier))
                    //{
                    //    Claim userIdClaim = new Claim(ClaimTypes.NameIdentifier, user.Id);
                    //    await _userManager.AddClaimAsync(user, userIdClaim);  // Add the claim to the user
                    //}
