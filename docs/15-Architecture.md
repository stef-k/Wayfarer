...
- Delay compares persisted `LocalTimestamp`, the client event instant stored in UTC, before DTO conversion to coordinate-local display time. `Timestamp` is server receipt time and remains the public/all-time summary date convention. Live state omits the cutoff.
...
Geoapify routing is a distinct adapter behind the proposal seam. Personal credential, capability verification, active selection, exact provider-native mode, and usage admission form the complete contact authority. Transport Profiles remain independent planning identity. Ordinary Segment Save validates the original protected proposal inside its Serializable transaction and atomically commits route provenance with other Segment edits, while the authenticated Mobile route endpoint returns but does not persist ad-hoc routes.
