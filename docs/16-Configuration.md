...
- Windows Development uses `LocalApplicationData/Wayfarer/{Data,Cache,Logs}` and system temp's `wayfarer` directory. Outside Development, all four roots must be explicitly configured **when requesting the authority**. Its lazy registration does not require these settings for existing native startup.
...
- **No current subsystem uses these targets yet.** Uploads, caches, thumbnails, logs, persisted paths, and Data Protection configuration/identity retain existing behavior. Subsystem adoption and native transition are separate #609 slices; this configuration does not migrate files or database values.
