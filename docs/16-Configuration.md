...
- [Admin storage display](images/admin-tile-storage.png) shows the compiled Razor view with disposable example values; this is a rendering preview, not production migration evidence.
...
Image cache storage and compatibility (#623)
- New proxied images use `StoragePaths.Images` (`Storage:CacheRoot/images`), `/var/cache/wayfarer/images` in Linux Production and the external per-user cache root in Development. New image writes do not require checkout/publish-root writability.
- `CacheSettings:ImageCacheDirectory` is deprecated and compatibility-only: absolute values remain absolute, relative values use the process working directory, and absent/blank values fall back to `<working-directory>/ImageCache`. Retain customized values until explicit migration no longer needs them.
- New `ImageCacheMetadata.FilePath` values are logical filenames: `<64-lowercase-hex-cache-key>.dat` initially, or `<cache-key>.<32-lowercase-hex-guid>.dat` after refresh. `ContentType` remains authoritative; `.dat` contains opaque image bytes. `ImageCacheStorage` validates exact key/reference agreement before filesystem access.
- Recognized absolute legacy files remain fresh/stale-readable in place. Successful refresh writes a new current-root generation, commits logical metadata, then removes old recognized bytes best-effort. Publish/metadata failure preserves the old row and file. No startup or ordinary read bulk-copies files or rewrites references.
- Foreign, outside-root, malformed and mismatched references fail closed without basename searches. Reads retain unsafe metadata for diagnosis or explicit migration; successful origin fetch can repair it without touching the external path. LRU commits metadata retirement before size accounting and safe file deletion.
- ImageCache remains rebuildable/excluded from routine backups (#533). Explicit #603/#604 migration may preserve compatible bytes: quiesce writes, classify through `ImageCacheStorage`, derive `CanonicalReference`, copy into the current root, verify size/integrity, rewrite target rows and verify fresh/stale hits without origin contact. This slice performs no real production conversion.
...
- The image proxy accepts only byte-validated single-frame JPEG, PNG, GIF and WebP. HTML, SVG/XML, malformed data and other decoder formats (including BMP/TIFF/PBM/TGA/QOI/ICO) are rejected regardless of upstream MIME. Animated GIF/WebP/APNG are rejected by the existing frame policy before full decode.
- Every accepted image, including pass-through and legacy cache bytes, has fixed decoded limits: 8,192 pixels per dimension, 12,000,000 pixels, exactly one frame, and a 64 MiB conservative estimate calculated as width × height × 4.
- `optimize=true` retains JPEG/PNG output (JPEG/WebP → JPEG, PNG/GIF → PNG). `optimize=false` preserves original bytes only after the same bounded validation, with canonical MIME derived from bytes. Fresh/stale legacy cache entries are validated before serving or warm-up short circuits; invalid entries are unusable misses, never stale fallback, and may remain inert until safe replacement or LRU. No migration or bulk purge is required.
- The image-origin client disables ambient/system proxies, validates every DNS answer before connecting to a pinned IP, and retains automatic redirects capped at 50 with HTTPS→HTTP downgrade rejection. Mixed public/private answers fail closed; same-origin reuse stays on the validated socket.
- Four process-wide origin workers coalesce requests for the same key. A new distinct key must acquire capacity immediately; no distinct-key queue is retained. Capacity or the shared 100-second total headers/body/cache deadline produces generic HTTP 503. Each request/job/series may cancel its own wait without cancelling other waiters; shared work owns its DI scope, dictionary entry and slot until it actually completes. Synchronous ImageSharp decode remains resource-bounded, not CTS-preemptible. Background refresh retains three attempts within a two-minute series with exponential delay/jitter.
- Safe responses retain cache diagnostics and public caching (default 90 days, 512 MiB disk target). Versioned HTTP ETags hash validated output; conditional 304 is considered only after safe content resolution. Disk keys are unchanged. Already-fresh browser copies cannot be remotely revoked by this server policy.
- Anonymous GET `/Public/ProxyImage?url=...` remains compatible with mobile WebView URLs and original-URL unwrapping. Safe cache hits bypass origin-miss admission; authenticated web principals retain their bypass. No bearer-only requirement is introduced. Disallowed initial URL/resource excess remains 400, invalid content/upstream non-success 404, and anonymous miss limiting 429; caller cancellation ends the wait without an image status.
