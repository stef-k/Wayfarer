...
Geoapify reverse geocoding and routing use separate cohesive adapters with fixed official endpoints, response/timeout bounds, exact normalization, and exception containment. Routing resolves administrator-owned provider-scoped stable Transport Profile mappings before shared-credit admission; labels are never interpreted as provider modes. The Mobile discovery owner loads one bounded coherent eligible catalog, performs protected readability once, revalidates generations, and hashes only non-secret authority fields. It never contacts a provider or enters admission/pacing. The mobile routing service fences discovery-aware requests before admission and returns neutral validated routes without persisting ad-hoc server state.
...
Personal generation and verification retain the provider pacing, rolling attempt rate, provider concurrency, global concurrency, retry, timeout, response-size, endpoint, and DNS-pinning authorities. Generation also retains the five-operations-per-user-per-minute window; personal verification does not consume that generation-only window. These budgets remain process-local, and personal credentials add no quota, billing, or distributed coordination layer.
