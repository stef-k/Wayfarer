...
- Each Quartz execution processes at most 10 wholly empty owned candidates in timestamp/ID order. After its workflow state commits, the existing authenticated SSE channel prompts the page to reload those durable counters. Permanent and not-yet-due attempts are skipped so poison rows cannot starve later Locations.
