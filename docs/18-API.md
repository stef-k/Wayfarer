...
`GET /api/mobile/routing/profiles` authenticates with the existing mobile token and returns at most 100 complete, ordered provider-neutral choices. Discovery is local-only: it performs no provider verification/contact, credit admission, reservation, consumption, or pacing. An `available` response includes an opaque versioned `authorityIdentity`; all other bounded discovery outcomes return an empty list and no identity.

`GET /api/mobile/routing/capability/{transportProfileId}` remains a no-contact per-profile check and returns an identity only after resolution and a fresh complete-catalog revalidation agree. `POST /api/mobile/routing/route` accepts a stable Wayfarer Transport Profile ID, origin/destination, at most three ordered anchors, and an optional discovery identity. Omission or JSON `null` preserves legacy behavior. A malformed supplied identity returns bounded `400 invalid-request`; a well-formed stale identity returns `200 authority-changed`. Discovery-aware requests freshly compare the complete eligible catalog before admission, before contact, during contact, and before successful publication, so a change to any eligible profile invalidates the accepted identity. Successful routes echo only the accepted current identity.
