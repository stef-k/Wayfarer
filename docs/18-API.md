...
Available estimates, including zero, become kilometres and Automatic minutes on Save. Missing estimates retain the canonical baseline under the matching token. An explicit Manual-duration edit during preview wins through ordinary normalization. Unchanged trusted-route saves preserve measurements and database provenance without a proposal token or current provider availability; replacement or clearing removes misleading route metadata. Ordinary non-proposal calculations otherwise retain their existing behavior.
...
`GET /api/mobile/routing/capability/{transportProfileId}` remains a no-contact check and accepts optional `providerMode` and `discoveryCatalogIdentity`. Updated clients explicitly submit a mode from the returned closed catalog. Omission preserves only the released-client compatibility path, which maps exact reviewed built-in Transport Profile keys and rejects all other keys before contact. A malformed identity returns bounded `400 invalid-request`; stale authority returns `200 catalog-changed` or `200 authority-changed`. Personal credential, verification, selection, catalog mode, and relevant Segment authority are fenced before admission, immediately before contact, through callback validation, and before publication.

Authenticated local discovery failures, including database, materialization, transaction, and protected-read failures, return `200 temporarily-unavailable` with an empty profile list and no identity. Caller cancellation still propagates. Server logs use only the bounded discovery-failure event and do not include exception diagnostics or authority data.

The client cannot submit a provider, endpoint, or credential. It may submit only a mode from the active provider's discovered catalog. Responses use bounded outcomes and may include validated geometry, metrics, normalized instructions, generation time, opaque personal-authority identities, linked attribution, and offline storage authority. Legacy provider-configuration response fields remain additive wire compatibility only and do not represent database authority. Ad-hoc routes are not persisted by Wayfarer. Public Segment JSON includes nullable current `transportProfileId`; retained `routeTransportProfileId` remains route provenance.
