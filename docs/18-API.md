...
`GET /api/mobile/routing/capability/{transportProfileId}` remains a no-contact per-profile check and additively returns the current authority identity when a complete discovery projection is available. `POST /api/mobile/routing/route` accepts a stable Wayfarer Transport Profile ID, origin/destination, at most three ordered anchors, and an optional discovery identity. Omission or JSON `null` preserves legacy behavior. A malformed supplied identity returns bounded `400 invalid-request`; a well-formed stale identity returns `200 authority-changed` before provider admission or contact. Successful discovery-aware routes echo the accepted identity.
