...
`GET /api/mobile/routing/capability/{transportProfileId}` remains a no-contact per-profile check and accepts optional `discoveryCatalogIdentity`. A malformed supplied value returns bounded `400 invalid-request`; a stale catalog or missing choice returns `200 catalog-changed`. Successful selection returns the accepted `DiscoveryCatalogIdentity` and a new `SelectedProfileAuthorityIdentity`. The catalog identity ends there. `POST /api/mobile/routing/route` accepts a stable Wayfarer Transport Profile ID, origin/destination, at most three ordered anchors, and optional `selectedProfileAuthorityIdentity`. Omission or JSON `null` preserves legacy behavior. A malformed supplied identity returns bounded `400 invalid-request`; a well-formed stale identity returns `200 authority-changed`. The selected identity is checked before admission, immediately before contact, through provider callback validation, and before publication. Unrelated catalog edits do not invalidate it; selected mapping, provider, credential, authorization, selection, or feature authority changes do. Successful routes echo only the accepted current selected identity.
...
The client cannot submit a provider, native mode, endpoint, or credential. Responses use bounded outcomes and may include validated geometry, metrics, normalized instructions, generation time, stable non-secret provider/configuration/mapping identity, linked attribution, and offline storage authority. Ad-hoc routes are not persisted by Wayfarer. Public Segment JSON now additively includes nullable current `transportProfileId`, projected only from the Segment's current selection; retained `routeTransportProfileId` remains route provenance.
