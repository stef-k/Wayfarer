...
`Segments.EstimatedDurationSource` is a required integer enum: `Automatic = 0` and `Manual = 1`. The database default is Automatic and `CK_Segments_EstimatedDurationSource` rejects every other value. Migration `20260802163750_AddSegmentMeasurementProvenance` classifies each legacy non-null duration as Manual and each null duration as Automatic without recalculating distance or duration. Downgrading removes the column and permanently loses provenance written after the upgrade; re-upgrading can only reclassify from duration nullability.
