...
public interface IVisitBackfillService
{
    ...
    Task<BackfillPreviewDto> PreviewAsync(string userId, Guid tripId, DateOnly? fromDate, DateOnly? toDate);
    ...
    Task<BackfillResultDto> ApplyAsync(string userId, Guid tripId, BackfillApplyRequestDto request);
    ...
    Task<BackfillResultDto> ClearVisitsAsync(string userId, Guid tripId);
}
