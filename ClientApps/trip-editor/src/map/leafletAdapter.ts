...
class TripEditorRetryTileLayer extends L.TileLayer {
    ...
    acquireTileFetchSlot(signal).then(acquired => {
      ...
      fetch(url, { signal })
        .then(response => {
          releaseTileFetchSlot();
          if (response.ok) {
            return this.loadTileBlob(response, tile, done, signal);
          }
  ...
  private scheduleRetry(url: string, tile: RetryTileImage, done: L.DoneCallback, attempt: number, signal: AbortSignal, retryAfterHeader?: string | null): void {
    if (attempt < this.maxRetries) {
      ...
      window.setTimeout(() => {
        if (!signal.aborted) {
          this.fetchWithRetry(url, tile, done, attempt + 1, signal);
        }
      }, delayMs);
      return;
    }
  ...
  private scheduleSlowRetry(url: string, tile: RetryTileImage, done: L.DoneCallback, signal: AbortSignal): void {
    ...
    const delayMs = jitter(retryAfterSeconds * 3 * 1000);
    window.setTimeout(() => {
      if (!signal.aborted) {
        this.fetchWithRetry(url, tile, done, this.maxRetries, signal);
      }
    }, delayMs);
  }
  ...
  private async loadTileBlob(response: Response, tile: RetryTileImage, done: L.DoneCallback, signal: AbortSignal): Promise<void> {
    ...
    tile.onerror = (event): void => {
      URL.revokeObjectURL(tile.src);
      done(event instanceof Error ? event : new Error('Tile image decode failed'), tile);
    };
    tile.src = URL.createObjectURL(blob);
  }
}

const createTripEditorTileLayer = (tilesUrl: string, options: L.TileLayerOptions): L.TileLayer =>
