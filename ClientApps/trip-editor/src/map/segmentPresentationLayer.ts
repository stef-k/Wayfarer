...
export const createSegmentPresentationLayer = (
  ...
  const addEntry = (presentation: EditorSegmentPresentation, active: boolean): void => {
    ...
    if (proposalSegmentId !== null && presentation.segmentId === proposalSegmentId) line.setStyle({ opacity: 0.40 });
  ...
  const setProposalEmphasis = (segmentId: Guid | null): void => {
    ...
    registry.forEach(entry => entry.line.setStyle({
      opacity: segmentId !== null && entry.presentation.segmentId === segmentId ? 0.40 : entry.normalOpacity
    }));
  };
