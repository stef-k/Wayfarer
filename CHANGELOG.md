...
### Fixed
- Bound SSE admission, sends, backlog and broadcast lifetime; retire empty channels safely and join heartbeat/send cleanup. Revalidate web/mobile group delivery with persisted PostgreSQL shared leases so durable revocation wins. Remove redundant Caddy negative flushing; preserve routes, payloads, authorization owners and heartbeat cadences. No database migration; mobile remote-disconnect recovery remains #674 (#657).
- Restrict generic SSE to exact public/live `location-update` streams with a server-owned channel prefix; reject private-channel aliases and legacy types before database or subscription work. Preserve per-event Timeline eligibility leases and dedicated stream contracts. No database migration (#656).
...
### Development
- Record the container/release architecture contract (#638): runtime/browser and database families, storage/secrets, immutable release/bundle identity, and explicit lifecycle boundaries. Architecture only; no Docker distribution or runtime behavior change.
